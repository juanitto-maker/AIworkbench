...
# aiwb — AI Workbench orchestrator (TUI if gum exists; falls back to text)
set -euo pipefail
...
# load envs if present (do not fail if missing)
...
gum_input() {
  if has gum; then
    ...
    read -r -p "${1:-}: " REPLY || true; printf '%s' "$REPLY"
  fi
}
...
set_project() { local p="$1"; [ -n "$p" ] || return 1; echo "$p" > "$AIWB/current.project"; mkdir -p "$AIWB_CODE_ROOT/$p/temp" "$AIWB_CODE_ROOT/$p/out"; }
set_task()    { local t="$1"; [ -n "$t" ] || return 1; echo "$t" > "$AIWB/current.task"; }
...
choose_model() {
  local default="${AIWB_DEFAULT_MODEL:-gemini}"
  local choice
  choice="$(gum_choose "gemini" "claude")"
  [ -z "$choice" ] && echo "$default" || echo "$choice"
}

choose_tier() {
  # Four clear options
  local c; c="$(gum_choose "Abort" "Basic" "Medium" "Best")"
  case "$c" in
    ""|"Abort") echo "" ;;
    Basic)  echo "basic" ;;
    Medium) echo "medium" ;;
